...
class RNN(object):
    ...
    def build_model(self):
        ...
        self.y  = tf.placeholder(tf.float32,[1,labNum])    # for training
        ...
        s = s[0,-1,:][tf.newaxis,:]#tf.squeeze(tf.Tensor.getitem(s,[0,s.get_shape()[1]-1,0],[1,1,hidNum]),[0])
        ...
        with tf.Graph().as_default():
            ...
            while running:
                ...
                if batch_x is not None:                    
                    ...
                    _,vld_f1 = evaluate(self,session,pred)
                    ...
                    print("[Epoch %d] f1:%.5f"  % (epoch,vld_f1))
                    total_err = 0
            ...
            vs,_ = evaluate(self,session,pred)            
            ##### Evaluation on test set
            if self.dataset.is_test:                
                ts,_ = evaluate(self,session,pred,eval_type="test")
            else:
                ts = 0
...
def evaluate(model,session,pred,eval_type="validation"):    
    ys = []
    rs = []
    while True:
        if eval_type=="validation":
            x_,y_ = model.dataset.next_valid()
        elif eval_type=="test":
            x_,y_ = model.dataset.next_test()
        if x_ is None:
            break

        #print(x_.shape)
        pd = session.run(pred,{model.x:x_,model.y:y_,model.l:[x_.shape[1]]})
        
        ys.append(np.argmax(y_,axis=1)[0])
        rs.append(pd[0])

        f1 = f1_score(ys,rs,average="micro")        
        s = classification_report(ys,rs,target_names=["Negative","Positive"],digits=5)
    return s,f1
